package frc.robot.motorcontrol.pathtracking;
...
import frc.robot.motorcontrol.utils.Utils;
...
public class LQRVelocitySteeringControl {
    ...
    private RealMatrix Q = MatrixUtils.createRealIdentityMatrix(5);
    private RealMatrix R = MatrixUtils.createRealIdentityMatrix(2);
    ...
    public RealMatrix solveDARE(
        ...
        RealMatrix X = Q;
        RealMatrix Xn = MatrixUtils.createRealMatrix(X.getRowDimension(), X.getColumnDimension());
        double eps = 0.1;

        for (int i = 0; i < 150; i++) {
            // A.T * X * A - A.T * X * B * INV(R + B.T * X * B) * B.T * X * A + Q
            RealMatrix AtX = A.transpose().multiply(X);
            RealMatrix BtX = B.transpose().multiply(X);
            RealMatrix firstTerm = A.multiply(AtX);
            RealMatrix inverse = MatrixUtils.inverse(R.add(B.multiply(BtX)));
            RealMatrix secondTerm = B.multiply(AtX).multiply(inverse).multiply(A.multiply(BtX));
            Xn = firstTerm.subtract(secondTerm.add(Q));

            double[][] XValues = X.getData();
            double[][] XnValues = Xn.getData();
            double max = 0.0;
            for (int j = 0; j < XValues.length; j++) {
                for (int k = 0; k < XValues[j].length; k++) {
                    double value = Math.abs(XnValues[j][k] - XValues[j][k]);
                    if (value > max) {
                        max = value;
                    }
                }
    ...
    public RealMatrix solveDiscreteTimeLQR(
        ...
        // (B.T * X * B + R) * (B.T * X * A)
        RealMatrix BtX = B.transpose().multiply(X);
        RealMatrix firstTerm = BtX.multiply(B).add(R);
        RealMatrix secondTerm = BtX.multiply(A);
        RealMatrix K = MatrixUtils.inverse(firstTerm).multiply(secondTerm);

        RealMatrix eigenMatrix = B.multiply(K);
        eigenMatrix = A.subtract(eigenMatrix);
    ...
    public int lqrSteeringControl(
        ...
        A.setEntry(0, 0, 1.0);
        A.setEntry(0, 1, timeIncrement);
        A.setEntry(1, 2, velocity);
        A.setEntry(2, 2, 1.0);
        A.setEntry(2, 3, timeIncrement);
        A.setEntry(4, 4, 1.0);

        RealMatrix B = MatrixUtils.createRealMatrix(5, 2);
        B.setEntry(3, 0, velocity / wheelBase);
        B.setEntry(4, 1, timeIncrement);

        RealMatrix K = solveDiscreteTimeLQR(A, B, Q, R);
        ...
        RealMatrix ustar = K.scalarMultiply(-1.0).multiply(x);
    ...
    public int calculateNearestIndex(double[] planX, double[] planY, double[] planYaw) {
        ...
        for (int i = 0; i < planX.length; i++) {
            distancesToPlanPoints.add(Math.pow((currentX - planX[i]), 2) + Math.pow((currentY - planY[i]), 2));
        }
        ...
        double angle = pi2pi(planYaw[index] - Math.atan2((planY[index] - currentY), (planX[index] - currentX)));
        if (angle < 0) {
